...
* ユーザーのログインセッションが破棄され、退会完了画面（`/deactivated`）へ遷移する。
...
5. **[システム]** ユーザーアカウントをシステムから完全に削除し、ログイン状態を解除（ログアウト）した上で、専用の退会完了画面（`/deactivated`）へ案内します。
...
## 7. 関連画面
* **一般ユーザー用**: 設定画面 (`/settings`)、退会完了画面 (`/deactivated`)
